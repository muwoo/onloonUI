<template>
  <div style="position: relative">
    <textarea
      @input="handleInput"
      v-bind="$props"
      class="lx-limit-textarea"
      :class="{'lx-limit-over': isOver}"
      :value="currentValue"
      >
    </textarea>
    <span class="lx-limit-text">
      {{isOver ? '已超出' : '还可以输入'}}<span class="lx-limit-num" :class="{'lx-limit-num-over': isOver}">{{num}}</span>个字
    </span>
  </div>
</template>
<script>
